...
import WomenCard from './womenCard.js';
...
export default function WomenCategory(){
    return(
        <div className="pop">
            ...
            <div className='pap'>
                {data.map((item)=>{
                    return <WomenCard id={item.id} category={item.category} name={item.name} img={item.image} newPrice={item.new_price} oldPrice={item.old_price} />
                })}
            </div>
