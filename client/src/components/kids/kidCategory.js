...
// import './ben.css';
import KidsCard from "./kidCard.js";
...
export default function KidsCategory(){
    return(
        <div className="pop">
            ...
            <div className='pap'>
                {data.map((item)=>{
                    return <KidsCard id={item.id} category={item.category} name={item.name} img={item.image} newPrice={item.new_price} oldPrice={item.old_price} />
                })}
            </div>
